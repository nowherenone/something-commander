...
export function NetworkConnections({ onClose, onConnected }: NetworkConnectionsProps): React.JSX.Element {
  ...
  const [savedSmb, setSavedSmb] = useState<Array<{ host: string; share: string; username: string; password: string; domain: string; label: string }>>([])
  ...
  const loadConnections = useCallback(async () => {
    ...
    if (Array.isArray(saved)) setSavedSmb(saved as typeof savedSmb)
  }, [])
  ...
  const handleConnect = useCallback(async () => {
    ...
    try {
      ...
      let pluginId: string = addingType!
      ...
      switch (addingType) {
        ...
        case 'smb': {
          ...
          if (saveConnection) {
            const entry = { host, share, username, password, domain, label }
  ...
  const handleLoadSaved = useCallback((conn: typeof savedSmb[0]) => {
  ...
  return (
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.dialog} onClick={(e) => e.stopPropagation()} style={{ width: 520, maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
        <div className={styles.dialogTitle}>Network Connections</div>
        <div className={styles.dialogBody} style={{ flex: 1, overflowY: 'auto' }}>
          ...
          {addingType ? (
            <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: 12 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {addingType === 'sftp' && (
                  <>
                    <Row label="Host:" value={host} onChange={setHost} onKeyDown={onKeyDown} autoFocus placeholder="hostname or IP" />
                    <Row label="Port:" value={port} onChange={setPort} onKeyDown={onKeyDown} placeholder="22" style={{ width: 80 }} />
                    <Row label="Username:" value={username} onChange={setUsername} onKeyDown={onKeyDown} placeholder="user" />
                    <Row label="Password:" value={password} onChange={setPassword} onKeyDown={onKeyDown} placeholder="password" type="password" />
                  </>
                )}
                {addingType === 's3' && (
                  <>
                    <Row label="Bucket:" value={bucket} onChange={setBucket} onKeyDown={onKeyDown} autoFocus placeholder="my-bucket" />
                    <Row label="Region:" value={region} onChange={setRegion} onKeyDown={onKeyDown} placeholder="us-east-1" />
                    <Row label="Access Key:" value={accessKeyId} onChange={setAccessKeyId} onKeyDown={onKeyDown} placeholder="AKIA..." />
                    <Row label="Secret Key:" value={secretAccessKey} onChange={setSecretAccessKey} onKeyDown={onKeyDown} placeholder="secret" type="password" />
                    <Row label="Label:" value={label} onChange={setLabel} onKeyDown={onKeyDown} placeholder="optional" />
                  </>
                )}
                {addingType === 'smb' && (
                  <>
                    <Row label="Host:" value={host} onChange={setHost} onKeyDown={onKeyDown} autoFocus placeholder="192.168.1.100" />
                    <Row label="Share:" value={share} onChange={setShare} onKeyDown={onKeyDown} placeholder="shared-folder" />
                    <Row label="Username:" value={username} onChange={setUsername} onKeyDown={onKeyDown} placeholder="user" />
                    <Row label="Password:" value={password} onChange={setPassword} onKeyDown={onKeyDown} placeholder="password" type="password" />
                    <Row label="Domain:" value={domain} onChange={setDomain} onKeyDown={onKeyDown} placeholder="optional" />
                    <Row label="Label:" value={label} onChange={setLabel} onKeyDown={onKeyDown} placeholder="optional" />
...
function Row({ label, value, onChange, onKeyDown, placeholder, type, autoFocus, style }: {
  label: string; value: string; onChange: (v: string) => void; onKeyDown: (e: React.KeyboardEvent) => void
  placeholder?: string; type?: string; autoFocus?: boolean; style?: React.CSSProperties
}): React.JSX.Element {
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
      ...
      <input
        ...
        placeholder={placeholder} className={styles.settingsInput}
        style={{ flex: 1, width: 'auto', ...style }}
      />
    </div>
  )
}
